...
def main():
    ...
    for label, color, label_name in [(0, "#2E86AB", "Factual"), (1, "#E74C3C", "Hallucination")]:
        ...
        ax.hist(subset, bins=80, alpha=0.6, label=label_name, color=color, 
                edgecolor="black", linewidth=0.5, density=True)
    
    ax.set_xlabel("Character Length", fontsize=12)
    ax.set_ylabel("Density", fontsize=12)
    ax.set_title("Answer Character Length Distribution by Class", fontsize=14, fontweight="bold")
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    
    mean_factual = df[df["label"] == 0]["char_count"].mean()
    mean_halluc = df[df["label"] == 1]["char_count"].mean()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "length_distribution_by_class.png"), dpi=300, bbox_inches="tight")
    print(f"Saved: {os.path.join(RESULTS_DIR, 'length_distribution_by_class.png')}")
    plt.close()
